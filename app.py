...
def save_manifest(manifest):
    ...
    try:
        ...
        logger.info(
            f"✓ Manifest saved successfully ({len(manifest.get('videos', []))} videos)"
        )
    except Exception as e:
...
def upload_background():
    ...
    make_default = request.form.get("makeDefault", "false").lower() in [
        "true",
        "1",
        "on",
    ]
    ...
    entry = {
        ...
        "description": description,
        "path": "/"
        + os.path.relpath(save_path, start=os.getcwd()).replace(os.path.sep, "/"),
...
def get_video(filename):
    ...
    manifest = load_manifest()
    video_entry = next(
        (v for v in manifest["videos"] if v["filename"] == filename), None
    )
    ...
    if os.path.exists(video_path):
        return send_file(
            video_path, as_attachment=True, download_name=filename, mimetype="video/mp4"
        )
    ...
    if video_entry:
        ...
        if os.path.exists(video_path):
            try:
                ...
                # Update manifest
                manifest["videos"] = [
                    v for v in manifest["videos"] if v["filename"] != filename
                ]
                save_manifest(manifest)
...
def generate():
    ...
    request.form.get("voice_provider", "auto")
    request.form.get("voice_model", "auto")
    request.form.get("video_length", "full")
...
def generate_long():
    ...
    try:
        ...
        if not green_screen_media and stories:
            logger.info(
                "📸 No green screen uploaded for stories, fetching from Pexels API..."
            )
            from pexels_helper import fetch_image_from_pexels

            first_headline = (
                stories[0].get("headline")
            ...
            if first_headline:
                ...
                if pexels_image:
                    ...
                    logger.info(
                        "⚠️ Pexels API unavailable or no image found; placeholder will be used"
                    )

        if not stories:
            return jsonify(
                {"error": "title and description required (or provide stories)"}
            ), 400
        ...
        logger.info(
            f"🎬 Starting long-form video generation: {preview_headline} (stories={len(stories)})"
        )
        ...
        for s in stories:
            ...
            if not h or not d:
                ...
                return jsonify(
                    {"error": "Each story requires headline and description"}
                ), 400
        ...
        try:
            ...
            return jsonify(
                {"status": "failed", "stage": "video_generation", "error": str(e)}
            ), 400
        ...
        return jsonify({"status": "failed", "error": str(e), "traceback": tb}), 500
...
def test_long():
    ...
    try:
        ...
        type("obj", (object,), {"get_json": lambda: test_data})()
        ...
        test_video_path = os.path.join(
            VIDEOS_DIR, "long", f"TEST_long_video_{timestamp}.mp4"
        )
        ...
        try:
            entry = add_to_manifest(
                video_path, test_headline, test_description, "english"
            )
        except Exception as e:
        ...
        return jsonify({"status": "test_failed", "error": str(e)}), 500
