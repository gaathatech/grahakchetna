...
def add_text_shadow(draw, text, position, font, shadow_offset=3):
    ...
    draw.text(
        ...
        text,
        font=font,
        fill=(*COLOR_SHADOW, 200),
    )


def create_boxed_text_image(
    text,
    fontsize=40,
    color=(255, 255, 255),
    bold=True,
    ...
    for line in lines:
        draw.text(
            (10 + shadow_offset, y + shadow_offset),
            line,
            font=font,
...
def create_text_image(
    ...
    if language in ["gujarati", "hindi"]:
        try:
            ...
            working_font = _find_working_font_for_text(
                text, fontsize, INDIC_FONT_PATHS + FONT_PATHS
            )
            if working_font:
                font = working_font
    lines = []
...
def create_ticker_text_image(
    text, fontsize=50, color=(255, 255, 255), bold=True, language="en"
):
    ...
    # Create image
    img = Image.new(
        "RGBA", (img_width + shadow_offset, img_height + shadow_offset), (0, 0, 0, 0)
    )
    draw = ImageDraw.Draw(img)

    # Draw shadow
    draw.text(
        (20 + shadow_offset, 15 + shadow_offset),
        text,
        font=font,
        fill=(*COLOR_SHADOW, 180),
    )
...
def create_right_content_box(
    text, fontsize=32, color=(255, 255, 255), bold=True, language="en"
):
...
def generate_video(
    ...
    try:
        if os.path.exists(shorts_bg_path):
            ...
            logger.warning(
                f"Shorts background not found: {shorts_bg_path}, using bg.mp4"
            )
            bg = (
        ...
        bg = VideoFileClip("assets/bg.mp4").resize((WIDTH, HEIGHT)).subclip(0, duration)

    overlay = (
        ColorClip((WIDTH, HEIGHT), color=COLOR_OVERLAY_BG)
        .set_opacity(
            0.15
            if layout_backgroundBlur == "light"
            else (
                0.25
                if layout_backgroundBlur == "medium"
                else (0.4 if layout_backgroundBlur == "heavy" else 0.08)
            )
        )
    ...
    ticker_img_path, ticker_height = create_ticker_text_image(
        headline, fontsize=50, color=(255, 255, 255), bold=True, language=language
    )
    ...
    def make_ticker_position(t):
        ...
        scroll_speed = (
            WIDTH + 4500
        )  # Total distance to scroll - increased for faster speed
        ...
        ticker_bg = (
            ...
            .set_position(
                (
                    ...
                    int(
                        headline_bar_y
                        + (headline_bar_height - (ticker_height + 20)) / 2
                    ),
                )
            )
            .set_duration(duration)
    ...
    if has_media:
        ...
        try:
            if media_path.lower().endswith((".mp4", ".avi", ".mov", ".mkv", ".webm")):
                ...
                media_clip = media_clip.resize(
                    (
                        right_content_width,
                        int(right_content_width * media_clip.h / media_clip.w),
                    )
                )
                media_clip = media_clip.subclip(0, min(media_clip.duration, duration))
                if media_clip.duration < duration:
                    media_clip = concatenate_videoclips(
                        [media_clip] * int(duration / media_clip.duration + 1)
                    ).subclip(0, duration)
            else:
                ...
                media_clip = media_img.resize(
                    (right_content_width, media_height)
                ).set_duration(duration)
            ...
            logger.warning(
                f"Failed to load media {media_path}: {e} - falling back to text box"
            )
            has_media = False
            use_text_box = True
    else:
    ...
    if use_text_box:
        ...
        desc_start_y = (
            headline_bar_y + headline_bar_height + 10
        )  # just below headline bar
        ...
        if desc_height > desc_box_height:
            logger.info(
                f"Description scrolling enabled (height {desc_height} > box {desc_box_height})"
            )
            ...
            def desc_make_frame(t):
                ...
                cropped = full_img.crop(
                    (0, y_scroll, desc_width, y_scroll + desc_box_height)
                )
    ...
    breaking_raw = "Grahak Chetna For more videos, Subscribe and stay Updated - YouTube : https://www.youtube.com/@GrahakChetna X (Twitter)  :  www.x.com/grahakchetna Facebook : www.facebook.com/grahakchetnanews Instagram : www.instagram.com/grahak.chetna"
    ...
    ai_label = ai_label.set_position((20, HEIGHT - 60)).set_duration(duration)
    ...
    clips = [
        ...
        right_bg_box
        if "right_bg_box" in locals() and right_bg_box is not None
        else None,
        right_content_clip,
        breaking_bar,
        breaking_bar_border,
    ...
    ending_bg = ColorClip((WIDTH, HEIGHT), color=(0, 0, 0)).set_duration(
        ending_duration
    )
    ...
    ending_text = ending_text.set_position("center").set_duration(ending_duration)

    ending_clip = CompositeVideoClip([ending_bg, ending_text]).set_duration(
        ending_duration
    )
